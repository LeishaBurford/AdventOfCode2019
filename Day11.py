...
def compute(instructions, inputSignal, currentIndex, outputSignals):
    ...
    while index < len(instructions):
        ...
        parameter1 = getParameter(
            instructions, index + 1, fullCode[2], relativeBase)
        parameter2 = getParameter(
            instructions, index + 2, fullCode[1], relativeBase) if opCode in needSecondParam else 0
        parameter3 = getParameter(
            instructions, index + 3, fullCode[0], relativeBase) if opCode in needThirdParam else 0
        if opCode == 1:
...
def paint(startingColour):
    ...
    while not halted:
        ...
        computerResponse = compute(
            opCodes, intputSignal, programPosition, outputSignals)
